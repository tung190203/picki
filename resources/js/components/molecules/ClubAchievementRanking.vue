<template>
    <div class="space-y-6">
        ...
        <div class="flex bg-gray-100 p-1 rounded-xl max-w-md mx-auto">
            <button @click="setMainType('rating')"
                class="flex-1 py-2 text-center text-sm font-semibold rounded-lg transition-all"
                :class="mainType === 'rating' ? 'bg-white text-[#D72D36] shadow-sm' : 'text-gray-500 hover:text-gray-800'">
                Điểm trình
            </button>
            <button @click="setMainType('achievement')"
                class="flex-1 py-2 text-center text-sm font-semibold rounded-lg transition-all"
                :class="mainType === 'achievement' ? 'bg-white text-[#D72D36] shadow-sm' : 'text-gray-500 hover:text-gray-800'">
                Thành tích
            </button>
        </div>
        ...
        <div v-else class="space-y-6">
            ...
            <div class="flex flex-col sm:flex-row items-center justify-between gap-4 border-b border-gray-100 pb-4">
                <!-- Sub-tabs: ⭐ Sao vs 🏆 Cúp -->
                <div class="flex items-center space-x-2 bg-gray-50 p-1 rounded-xl w-full sm:w-auto">
                    <button @click="setSubType('star')"
                        class="px-4 py-2 rounded-lg text-xs font-bold transition-colors flex items-center gap-1.5"
                        :class="subType === 'star' ? 'bg-[#D72D36] text-white' : 'text-gray-600 hover:bg-gray-200'">
                        Kèo đấu
                    </button>
                    <button @click="setSubType('cup')"
                        class="px-4 py-2 rounded-lg text-xs font-bold transition-colors flex items-center gap-1.5"
                        :class="subType === 'cup' ? 'bg-[#D72D36] text-white' : 'text-gray-600 hover:bg-gray-200'">
                        Giải đấu
                    </button>
                </div>
                ...
                <div class="flex items-center space-x-1.5 overflow-x-auto w-full sm:w-auto custom-scrollbar pb-1 sm:pb-0">
                    <button v-for="tf in timeFrames" :key="tf.id" @click="setTimeFrame(tf.id)"
                        ...
                        :class="timeFrame === tf.id ? 'bg-gray-800 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'">
                        {{ tf.name }}
                    </button>
                </div>
            </div>
            ...
            <!-- Loading State -->
            <div v-if="achievementLoading" class="py-12 text-center text-gray-400">
                Đang tải bảng xếp hạng thành tích...
            </div>

            <!-- Empty State -->
            <div v-else-if="!achievementList.length" class="py-12 text-center text-gray-400">
                    ...
                    <div v-if="achievementTopThree[1]" class="flex flex-col items-center">
                        <div class="relative mb-2">
                            <img :src="achievementTopThree[1].avatar_url || defaultAvatar"
                                class="w-12 h-12 sm:w-16 sm:h-16 rounded-full object-cover border-2 border-gray-300 shadow-md" />
                            <span class="absolute -bottom-1 -right-1 bg-gray-200 text-gray-700 text-xs font-bold w-5 h-5 rounded-full flex items-center justify-center border border-white">2</span>
                        </div>
                        <h4 class="font-bold text-xs sm:text-sm text-gray-800 truncate max-w-[90px] sm:max-w-[120px] text-center">
                            {{ achievementTopThree[1].name }}
                        </h4>
                        <span v-if="achievementTopThree[1].is_virtual" class="text-[9px] bg-purple-100 text-purple-600 font-bold px-1 rounded mt-0.5">ẢO</span>
                        <div class="text-xs font-extrabold text-[#D72D36] mt-1">
                    ...
                    <div v-if="achievementTopThree[0]" class="flex flex-col items-center">
                        <div class="relative mb-2">
                            <img :src="achievementTopThree[0].avatar_url || defaultAvatar"
                                class="w-16 h-16 sm:w-20 sm:h-20 rounded-full object-cover border-2 border-amber-400 shadow-lg ring-4 ring-amber-100" />
                            <span class="absolute -bottom-1 -right-1 bg-amber-400 text-white text-xs font-bold w-6 h-6 rounded-full flex items-center justify-center border border-white">1</span>
                        </div>
                        <h4 class="font-bold text-sm sm:text-base text-gray-900 truncate max-w-[100px] sm:max-w-[140px] text-center">
                            {{ achievementTopThree[0].name }}
                        </h4>
                        <span v-if="achievementTopThree[0].is_virtual" class="text-[9px] bg-purple-100 text-purple-600 font-bold px-1 rounded mt-0.5">ẢO</span>
                        <div class="text-sm font-extrabold text-[#D72D36] mt-1">
                            {{ achievementTopThree[0].total_points }} {{ subType === 'star' ? '⭐' : '🏆' }}
                        </div>
                        <div class="text-[11px] text-gray-600 font-medium mt-0.5 space-x-1">
                    ...
                    <div v-if="achievementTopThree[2]" class="flex flex-col items-center">
                        <div class="relative mb-2">
                            <img :src="achievementTopThree[2].avatar_url || defaultAvatar"
                                class="w-12 h-12 sm:w-16 sm:h-16 rounded-full object-cover border-2 border-amber-700/40 shadow-md" />
                            <span class="absolute -bottom-1 -right-1 bg-amber-700 text-white text-xs font-bold w-5 h-5 rounded-full flex items-center justify-center border border-white">3</span>
                        </div>
                        <h4 class="font-bold text-xs sm:text-sm text-gray-800 truncate max-w-[90px] sm:max-w-[120px] text-center">
                            {{ achievementTopThree[2].name }}
                        </h4>
                        <span v-if="achievementTopThree[2].is_virtual" class="text-[9px] bg-purple-100 text-purple-600 font-bold px-1 rounded mt-0.5">ẢO</span>
                        <div class="text-xs font-extrabold text-[#D72D36] mt-1">
                ...
                <div class="divide-y divide-gray-100 border border-gray-100 rounded-xl overflow-hidden shadow-sm">
                    <div v-for="item in achievementList" :key="item.user_id || item.virtual_member_id || item.name"
                        ...
                        <div class="flex items-center space-x-3">
                            <span class="font-bold text-sm text-gray-400 w-6 text-center">{{ item.rank }}</span>
                            <img :src="item.avatar_url || defaultAvatar" class="w-10 h-10 rounded-full object-cover border border-gray-200" />
                            <div>
                                <div class="flex items-center space-x-1.5">
                                    <span class="font-semibold text-sm text-gray-800">{{ item.name }}</span>
                                    <span v-if="item.is_virtual" class="text-[9px] bg-purple-100 text-purple-700 font-bold px-1.5 py-0.5 rounded">ẢO</span>
                                </div>
                                <div class="text-xs text-gray-400 space-x-2 mt-0.5">
                                    <span>🥇 {{ item.gold }}</span>
                        ...
                        <div class="font-extrabold text-sm text-[#D72D36] flex items-center gap-1">
                            {{ item.total_points }}
