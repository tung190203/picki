    ...
    >
      <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4 overflow-hidden">
        ...
        <div class="flex items-center justify-between px-5 py-4 border-b border-gray-100">
          <h3 class="font-bold text-gray-900">Chia sẻ kèo đấu</h3>
          ...
          <div ref="cardRef" class="bg-white rounded-xl border border-gray-200 overflow-hidden shadow-sm">
            <div class="relative">
              ...
              <div v-else class="w-full h-48 bg-gradient-to-br from-[#BA110B] to-[#520011] flex items-center justify-center">
                <span class="text-white text-7xl font-bold opacity-30">{{ mini.name?.charAt(0) }}</span>
              </div>
              <div class="absolute top-3 left-3 flex gap-2">
                <span v-if="mini.sport" class="bg-black/60 text-white text-xs px-2 py-1 rounded-full backdrop-blur-sm flex items-center gap-1">
                  <img v-if="mini.sport.icon" :src="mini.sport.icon" class="w-3 h-3" :alt="mini.sport.name" />
                  {{ mini.sport.name }}
                </span>
            ...
            <div class="p-3">
              <h4 class="font-bold text-gray-900 text-base leading-tight mb-2 line-clamp-2">{{ mini.name }}</h4>
              <div class="flex items-center gap-3 text-xs text-gray-500">
                <span v-if="mini.competition_location" class="flex items-center gap-1">
                  <MapPinIcon class="w-3 h-3" />
                  {{ mini.competition_location.name }}
                </span>
                <span v-if="mini.start_time" class="flex items-center gap-1">
                  <CalendarDaysIcon class="w-3 h-3" />
                  {{ formatMiniEventDate(mini.start_time) }}
                </span>
              </div>
...
const statusText = computed(() => {
  ...
  const status = props.mini.status
  if (status === 1) return 'Nháp'
  if (status === 2) return 'Mở'
  if (status === 3) return 'Đóng'
  if (status === 4) return 'Hủy'
  return ''
})
...
const shareTitle = computed(() => props.mini?.name || '')
const shareText = computed(() => {
  const m = props.mini
  if (!m) return ''
  const location = m.competition_location?.name || ''
  const time = m.start_time ? formatMiniEventDate(m.start_time) : ''
  const parts = [location, time].filter(Boolean)
  return parts.length ? `🎾 ${m.name} - ${parts.join(' • ')}` : `🎾 ${m.name}`
})

function formatMiniEventDate(dateStr) {
  if (!dateStr) return ''
