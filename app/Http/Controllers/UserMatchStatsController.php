...
class UserMatchStatsController extends Controller
{
    ...
    public function matchesBySportId(Request $request)
    {
        ...
        foreach ($minis as $mini) {
            ...
            if ($isSolo) {
                ...
                $allMatches->push([
                    'type_text' => $this->getTypeText('mini_tournament', $mini->miniTournament->play_mode ?? null),
                    'format' => 'solo',
                    'id' => $mini->id,
    ...
    private function getTypeText(string $type, mixed $playMode = null): string
    {
        ...
        return match ($mode) {
            MiniTournament::PLAY_MODE_CASUAL => 'Kèo vui vẻ',
            MiniTournament::PLAY_MODE_COMPETITION => 'Kèo thi đấu',
            MiniTournament::PLAY_MODE_PRACTICE => 'Kèo luyện tập',
            default => 'Kèo đấu',
        };
    }
}
