...
class AuthController extends Controller
{
    ...
    public function me(Request $request)
    {
        ...
        $user->vn_rank = $this->getVNRankConsistent($user, 1);
        return ResponseHelper::success(new UserResource($user), 'Lấy thông tin người dùng thành công', 200);
    }

    /**
     * Tính vn_rank nhất quán với leaderboard: COUNT(users có score cao hơn) + 1,
     * có loại trừ email giống leaderboard.
     */
    private function getVNRankConsistent(User $user, int $sportId): ?int
    {
        ...
        $userTotalMatches = $user->getTotalMatches($sportId);
        ...
        $totalMatchesRaw = "(
            (
                ...
                WHERE tm.user_id = users.id AND t.sport_id = %d AND m.status = 'completed'
            ) + (
                SELECT COUNT(DISTINCT m.id) FROM matches m
                JOIN tournament_types tt ON m.tournament_type_id = tt.id
                JOIN tournaments t ON tt.tournament_id = t.id
                ...
                WHERE mtm.user_id = users.id AND mnt.sport_id = %d AND mm.status = 'completed'
            ) + (
                SELECT COUNT(DISTINCT mm.id) FROM mini_matches mm
                JOIN mini_tournaments mnt ON mm.mini_tournament_id = mnt.id
